...
def fetch_price_soup(rid):
  ...
  try: 
    ...
    logging.debug(f'Unable to reach prices for rid: {rid}')
  soup = BeautifulSoup(resp.text, features='lxml')
...
def fetch_df_players(engine, num_processes=10):
  ...
  try:
    ...
    logging.debug(f'Current pid: {current_pid}')
  ...
  logging.info(f'Number of players to collect: {total_pids}')
...
def fetch_df_prices(engine, num_processes=10):
  ...
  # Number of players to collect
  total_rids = len(rids)
  logging.debug(f'Number of players to collect prices for: {total_rids}')
...
def fetch_data():
  ...
  logging.debug('Done fetching players.')
  ...
  logging.debug('Done fetching prices.')
